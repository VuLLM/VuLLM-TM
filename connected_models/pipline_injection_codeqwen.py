...
def main(path_testset, model_huggingface_path, all_vulgen, output_dir):
    ...
    test = Prepare_dataset_with_unique_lines.get_testset_for_eval_for_only_decoder(tokenizer, path_testset, None)
    # test = test[:10]
    eos = tokenizer.eos_token
    test['prompt'] = test.apply(lambda row: f"""function:\n{row['inputs']}\nInstruction:\n{row['outputs']}{eos}""", axis=1)
    with open('connected_models/indexes_to_delete_from_test_codeqwen.pkl', 'rb') as file:
        indexes_to_delete = pickle.load(file)
