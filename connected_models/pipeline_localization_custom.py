...
def prepare_data(path, tokenizer):
    data = []
    ...
    tokenized_func = tokenizer(df['nonvul'].tolist(), return_tensors='pt', padding='max_length', truncation=True)
...
def main(path_testset, model_huggingface_path, output_dir):
    ...
    tokenized_func = prepare_data(path_testset, tokenizer)
    trainer = get_trainer(model, tokenizer, tokenized_func)
    local_res = get_prediction(trainer, tokenizer)
    df = pd.DataFrame(local_res, columns=['local_res'])
...
if __name__ == "__main__":
    ...
    main(args.path_testset, args.model_huggingface_path, args.output_dir)